...
fn guardian_elicitation_review_request_builds_mcp_tool_call() {
    ...
    let GuardianElicitationReview::Review(guardian_request) =
...
fn guardian_elicitation_review_request_defaults_missing_tool_params() {
    let request = form_request(guardian_meta(None));
...
fn guardian_decisions_map_to_elicitation_responses_without_session_state() {
    assert_eq!(
        mcp_elicitation_response_from_guardian_decision_parts(ReviewDecision::Approved, None),
        ElicitationResponse {
    ...
    assert_eq!(
        ...
        ElicitationResponse {
            ...
            content: None,
            meta: Some(json!({ "message": "Denied by Guardian" })),
        }
    );
    assert_eq!(
        mcp_elicitation_response_from_guardian_decision_parts(ReviewDecision::TimedOut, None),
        ElicitationResponse {
            action: ElicitationAction::Decline,
            content: None,
            meta: Some(json!({
    ...
    assert_eq!(
        mcp_elicitation_response_from_guardian_decision_parts(ReviewDecision::Abort, None),
        ElicitationResponse {
